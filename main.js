...
// Stato power-up timer Pac-Man: deve essere esportato per player.js
...
function initZoomControls() {
    ...
    btnIn.addEventListener('click', (e) => { e.stopPropagation(); updateZoom(0.1); });
    btnOut.addEventListener('click', (e) => { e.stopPropagation(); updateZoom(-0.1); });
    btnIn.addEventListener('touchstart', (e) => { e.preventDefault(); e.stopPropagation(); updateZoom(0.1); }, { passive: false });
    btnOut.addEventListener('touchstart', (e) => { e.preventDefault(); e.stopPropagation(); updateZoom(-0.1); }, { passive: false });
}
...
async function startGame() {
    ...
    safePlayAudio(sfx.beginLevel);
    document.body.classList.add('game-active');
    if (tapOverlay) { tapOverlay.classList.remove('show'); setTimeout(() => { tapOverlay.style.display = 'none'; }, 300); }
