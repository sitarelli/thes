...
export function setBackgroundForLevel(levelNumber) {
    ...
    if (bgImage.src.endsWith(newSrc)) return;
    const img = new Image();
    img.src = newSrc;
...
export function initRenderer(canvasElement, contextElement, spritesData) {
    ...
    if (isMobile) {
        ...
        Object.defineProperty(ctx, 'shadowBlur', { set: function(val) { originalSetter.call(this, 0); } });
    }
}
...
function drawPlayer() {
    ...
    if (player.isDying) {
        ...
        img = sprites.thes;
        if (!img || !img.complete) return;
        if (player.damageCooldown > 0) { if (Math.floor(Date.now() / 100) % 2 === 0) return; }
    ...
    if (player.isDying) dy += 3 * config.zoom;
    ...
    if (player.isInLava && !player.isDying) {
        ...
        const intensity = 0.5 + Math.sin(Date.now() * 0.01) * 0.2;
        ctx.fillStyle = `rgba(255, 50, 0, ${intensity * 0.6})`;
        const unTerzoW = dw / 3;
        if (player.facing === -1) ctx.fillRect(unTerzoW, 0, unTerzoW, dh);
        else ctx.fillRect(dx + unTerzoW, dy, unTerzoW, dh);
    }
    ctx.restore();
}
...
function drawVictoryMessage() {
    ...
    const layers = [
        ...
        { color: '#00ffff', offset: 4 },
        { color: '#ffff00', offset: 2 },
        { color: '#00ff41', offset: 0 },
    ];
    ...
    const alpha = Math.sin(time * 0.1) * 0.3 + 0.7;
