...
// === 1. MOVIMENTO ORIZZONTALE E COLLISIONI LATERALI ===
    ...
    lavas.forEach(l => {
        if (rectIntersect(player, l, false)) {
            touchingLava = true;
    ...
    items.forEach(item => { 
        if (!item.taken && rectIntersect(player, item, false)) { 
            ...
            else if (item.type === 'flag') { gameState.flags++; gameState.hasFlag = true; playSound('bonus'); }
...
function drawGlowingTrigger(x, y, w, h, color, text, pulseOffset = 0) {
    ...
    ctx.globalAlpha = 1;
    if (!isSamsungInternet) ctx.shadowBlur = 15 * pulse;
    ctx.fillStyle = color;
